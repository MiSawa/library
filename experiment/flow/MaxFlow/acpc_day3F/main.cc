...
struct Instance {//{{{
    ...
    friend istream &operator>>(istream &is, Instance &in){//{{{
        int k, n, m;
        is >> k >> n >> m;
        in.S = n, in.T = in.S+1;
        auto toId = [&](int u){
    ...
    template<class MF>
    typename MF::Flow solve() const {
        using F = typename MF::Flow;
        F res = -1;
        MF mf;
        for(size_t i = 0; i < es.size(); ++i){
            for(size_t j = 0; j < es.size(); ++j){
                const F cap = (i == j) ? 1LL<<30 : es[j].cap;
                mf.add_edge(es[j].u, es[j].v, cap);
                mf.add_edge(es[j].v, es[j].u, cap);
            }
            res = max(res, mf.run(S, T));
            if(i >= 100) break;
        }
        return res;
    }
...
int main(int argc, const char *argv[]){
    ...
    */
    ...
    for(int i = 1; i < argc; ++i){
        ...
        fstream fs(argv[i]);
        fs >> instance;
        string casename = stringbuilder() << argv[i] << "  (" << instance.S << ", " << instance.es.size() << ")";
        checker.reset();
