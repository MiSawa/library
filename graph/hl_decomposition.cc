...
struct HandlerAdd{//{{{
    ...
    val_type update(const val_type &old, const opr_type &op, const int u){
        return old + op;
    }
...
struct HLSegTree{//{{{
    ...
    struct Segtree{//{{{
        ...
        vector<int> path;
        Handler &hdl;
        vector<val_type> tree;
        Segtree(const vector<int> &path, Handler &hdl) : offset(calc_n(path.size())-1), path(path), hdl(hdl){//{{{
            const int n = calc_n(path.size());
            tree.assign(n * 2, hdl.default_value());
            for(int i = 0; i < path.size(); ++i) tree[offset + i] = hdl.default_value(path[i]);
        ...
        val_type build(const int l, const int r, const int k, const int ll, const int rr){//{{{
            ...
            if(r <= mm || mm <= l) return tree[k] = build(l, r, k*2 + (r <= mm ? 1 : 2), ll, mm);
            const auto lv = build(l, r, k*2+1, ll, mm);
            const auto rv = build(l, r, k*2+2, mm, rr);
            return tree[k] = hdl.merge(
                lv, get_path(l, r, ll, mm),
                rv, get_path(l, r, mm, rr));
        }//}}}
        void update(const int i, const opr_type &op){//{{{
            update(i, op, 0, 0, offset + 1);
        }//}}}
        const val_type &update(const int i, const opr_type &op, const int k, const int ll, const int rr){//{{{
            if(i+1 <= ll || rr <= i) return tree[k];
            if(i <= ll && rr <= i+1) return tree[k] = hdl.update(tree[k], op, path[i]);
            const int mm = (ll + rr) >> 1;
            const auto &lv = update(i, op, k*2+1, ll, mm);
            if(path.size() <= mm) return tree[k] = lv;
            const auto &rv = update(i, op, k*2+2, mm, rr);
            return tree[k] = hdl.merge(
                lv, get_path(ll, mm),
                rv, get_path(mm, rr));
        }//}}}
        void set(const int i, const opr_type &op){//{{{
            update(i, op, 0, 0, offset + 1);
        }//}}}
        const val_type &set(const int i, const val_type &val, const int k, const int ll, const int rr){//{{{
            if(i+1 <= ll || rr <= i) return tree[k];
            if(i <= ll && rr <= i+1) return tree[k] = val;
            const int mm = (ll + rr) >> 1;
            const auto &lv = set(i, k*2+1, ll, mm);
            if(path.size() <= mm) return tree[k] = lv;
            const auto &rv = set(i, k*2+2, mm, rr);
            return tree[k] = hdl.merge(
                lv, get_path(ll, mm),
                rv, get_path(mm, rr));
        }//}}}
    ...
    void update(int u, const opr_type &op){//{{{
        const int id = hdl.hl.nodes[u].path;
        const int plen = hdl.hl.paths[id].size();
        int i = hdl.hl.nodes[u].pos;
        segs[2*id].update(i, op);
        segs[2*id+1].update(plen - i - 1, op);
    }//}}}
    void set(int u, const val_type &val){//{{{
        const int id = hdl.hl.nodes[u].path;
        const int plen = hdl.hl.paths[id].size();
        int i = hdl.hl.nodes[u].pos;
        segs[2*id].set(i, val);
        segs[2*id+1].set(plen - i - 1, val);
    }//}}}
...
bool solve(){
    ...
    HLSegTree<HandlerAdd> hl_a(hdl_a);
    HLSegTree<HandlerMul> hl_m(hdl_m);
    ...
    rep(_, Q){
        ...
        if(op == 'U'){
            ...
            hl_a.update(u, {fib[k+1], fib[k]});
            hl_m.update(u, {fib[k+1], fib[k]});
        }else{
            int u, v; reader(u, v);
            int p = tree.lca(u, v);
