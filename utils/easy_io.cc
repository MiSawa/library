...
class AwesomeIO{//{{{
    ...
    template<class TPL, size_t n = std::tuple_size<typename std::remove_reference<TPL>::type>::value, size_t i = 0> struct rec_tuple{//{{{
        template<class Tpl>
        inline static AwesomeIO &read(AwesomeIO &io, Tpl &&tpl){
            io >> std::get<i>(std::forward<Tpl>(tpl));
            return rec_tuple<TPL, n, i+1>::read(io, tpl);
        }
        template<class T, class ...Args>
        inline static T construct(AwesomeIO &io, Args &&...args){
    ...
    explicit inline operator bool() const { return static_cast<bool>(is); }
    explicit inline operator bool() { return static_cast<bool>(is); }
    inline const bool operator!() const { return !is; }
    inline const bool operator!() { return !is; }
    ...
    template<class Tpl> inline auto operator>>(Tpl &&tpl) -> decltype(std::tuple_size<typename std::remove_reference<Tpl>::type>::value, *this){//{{{
        return rec_tuple<Tpl>::read(*this, std::forward<Tpl>(tpl));
    }//}}}

    template<class T = OManip> inline AwesomeIO &operator<<(const T &x) { os << x; return *this; }
    ...
    struct ItIn {//{{{
        template<class T> struct it_t : public std::iterator<std::input_iterator_tag, T>{//{{{
            ...
            bool has_val;
            T val;
            inline it_t(AwesomeIO &io, size_t rest) : io(io), rest(rest), has_val(false){}
        ...
        const size_t n;
        AwesomeIO &io;
        ItIn(const size_t n, AwesomeIO &io) : n(n), io(io){}

        template<class C, class T, class ...>
        inline C construct(){ return C(it_t<T>(io, n), it_t<T>(io, 0)); }

        template<template<class, class, class...> class C, class T, class U, class...Ts>
        inline operator C<T, U, Ts...>(){ return construct<C<T, U, Ts...>, T, U, Ts...>(); }
        template<template<class, size_t> class C, class T, size_t N>
        inline operator C<T, N>(){ return construct<C<T, N>, T>(); }
    };//}}}
    ...
    template<class ...Args>
    inline Builder<Args...> build(){ return Builder<Args...>(*this); }
    ...
    inline AwesomeIO &operator<<(Join<C, D> &&j){//{{{
        bool first = true;
    ...
    inline AwesomeIO &operator<<(const Cond<A, B> &c){//{{{
